...
for videoid in VideoIndex:
        ...
        if fsize > 30:
            for tW in timeWindow:
                mkdir(str(tW) + '_ShiftTile_Opt/' + str(videoid) + '/')
                ...
                for s in range(0, totalLength, tW):
                    ...
                    D = np.zeros((traj_count, traj_count),dtype=int)
                    ...
                    for k in range(6, max_cluster):
                        ...
                        for traj, cluster in zip(traj_lst, cluster_lst):
                            if cluster == -1:
                                ss = traj.shape
                                data = np.zeros((ss[0], 2),dtype=int)
                                td = 0
                                for traj_d in traj:
                        ...
                        M = np.zeros((clusterNumber, 30, H, W),dtype=int)
                        for data_t, cluster in zip(data_total, cluster_lst):  #求并集
                            if cluster != -1:
                                t = 0
                                for d in data_t:
                                    temp = function(height, width, basic_height, basic_width, int(d[1]), int(d[0]), V_height, V_width)
                                    M[cluster, t, :, :] = M[cluster, t, :, :] + function(height, width, basic_height,
                                                                                         basic_width, int(d[1]), int(d[0]),
                                                                                         V_height, V_width)
                                    t = t + 1
                        ...
                        cover_ratio = tiles / totalTiles #计算对于所有用户1s内的覆盖率，越大越好
                        uncover_ratio = uncover_tiles / uncover_totalTiles
                        cv[k-1] = 1 - cover_ratio + uncover_ratio
                        ucv[k-1] = uncover_ratio



                    min_cv = 1
                    for k in range(0, max_cluster-1):  # 2,12 0,10
                        if cv[k] < min_cv:
                            min_cv = cv[k]
                            final_clusterNumber = k + 1
                            final_coverRatio = cv[k]
                            final_uncoverRatio = ucv[k]

                    final_clusterNumber =6

                    '''
                        diff[k-2] = cv[k-2]-cv[k-1]
                        if (  diff[k-2]>0 and diff[k-2] < 0.03 and cv[k-2] < 0.4) or (k == 10):
                            final_clusterNumber = k-1 #分类数量要比下标号大1
                            final_coverRatio = cv[k-2]
                            final_uncoverRatio = ucv[k-2]
                            break
                  '''
                    ...
                    for k in range(final_clusterNumber, final_clusterNumber + 1):
                       ...
                       # mkdir('kMeansResult/' + str(videoid) + '/')
                       # scipy.io.savemat('kMeansResult/' + str(videoid) + '/' + str(s) + '.mat',{'clusterLst':cluster_lst})
                        cluster_lst = \
                        ...
                        output_N_PerUser = np.zeros((traj_count, 30, H, W),dtype=int) #包含每个用户的轨迹
                        tra = 0
                        for data_t, cluster in zip(data_total, cluster_lst):  # 求并集
                            if cluster != -1:
                                t = 0
                                for d in data_t:
                                    temp = function(height, width, basic_height, basic_width, int(d[1]), int(d[0]),
                                                    V_height, V_width)
                                    output_N_PerUser[tra, t, :, :] = function(height, width, basic_height, basic_width, int(d[1]), int(d[0]),
                                                    V_height, V_width)
                                    M[cluster, t, :, :] = M[cluster, t, :, :] + function(height, width, basic_height,
                                                                                         basic_width, int(d[1]),
                                                                                         int(d[0]),
                        ...
                        for data_t, cluster in zip(data_total, cluster_lst):  # 求像素浪费率
                            if cluster != -1:
                                ...
                                for d in data_t:
                                    ...
                                    output_M[cluster, t, :, :] = temp2.copy() #不带频度的
                                    temp3 = 1 - temp2  # 未使用的块的补集
                                    temp3 = temp3.astype(int)
                                    temp3 = np.bitwise_and(temp, temp3)  # 为未覆盖的块
                    ...
                    outputtilineMethor_PerUser.append(output_N_PerUser)
                    tilingClusterNumber[seconds] = final_clusterNumber
                    tilingPerformace[seconds] = final_coverRatio
                    tilingUncoverRatio[seconds] = final_uncoverRatio
                    outputCluster.append(cluster_lst)
                    seconds = seconds + 1
                    print(str(seconds) + ' of ' + str(math.ceil(totalLength/30)) + ' ' + str(videoid))


                matData2 = sio.savemat(str(tW) + '_ShiftTile_Opt/' + str(videoid) + '.mat', \
                            {'Method':  outputtilingMethod, 'uR':userRange, 'Method_Peruser': outputtilineMethor_PerUser, 'Method_Frequency': tilingMethod, 'clusterLst': outputCluster, 'clusterNumber': tilingClusterNumber},do_compression=True)
