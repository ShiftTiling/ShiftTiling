%% Running Flare storage encoding
clear all;
close all;
clc;
            ...
            for qp =22:5:42
                for i = 1:mm
                    for j = 1:nn
                        ...
                        fname_i = [outputPath,'outputVideo',Codec_Path,'/Flare/',num2str(videoid),'/',num2str(videoid),'_',num2str(seconds-1),'_',num2str(temp(1)),'_',num2str(temp(2)),'_',num2str(temp(3)),'_',num2str(temp(4)),'_',num2str(qp), '.mp4'];
                        if ~exist(fname_i,'file')
                            mkdir([outputPath,'outputVideo',Codec_Path,'/Flare/',num2str(videoid),'/'])
                            if Codec == 265
                                command = ['ffmpeg -i ',inputPath,'sourceVideo\',num2str(videoid),'_',num2str(seconds-1),'.mp4 -an  -c:v libx265  -x265-params qp=',num2str(qp),':keyint=30:fps=30 -vf crop=',num2str(temp(4)),':',num2str(temp(3)),':',num2str(temp(2)),':',...
                                    num2str(temp(1)),' ',outputPath,'outputVideo',Codec_Path,'/Flare/',num2str(videoid),'\',num2str(videoid),'_',num2str(seconds-1),'_',num2str(temp(1)),'_',num2str(temp(2)),'_',num2str(temp(3)),'_',num2str(temp(4)),'_',num2str(qp), '.mp4'];
                            else
